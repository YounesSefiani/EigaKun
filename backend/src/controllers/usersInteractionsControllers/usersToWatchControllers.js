...
const readUserToWatchMovies = async (req, res, next) => {
  try {
    const userId = req.params.id;

    const movies = await tables.usersHasToWatch.readToWatchMovies(userId);
    ...
    return res.json({
      ...
      toWatchMovies: movies.toWatchMovies,
    });
  } catch (err) {
    return next(err);
  }
...
const readUserToWatchSeries = async (req, res, next) => {
  try {
    const userId = req.params.id;

    const series = await tables.usersHasToWatch.readToWatchSeries(userId);
    ...
    return res.json({
      ...
      toWatchSeries: series.toWatchSeries,
    });
  } catch (err) {
    return next(err);
  }
...
const addToWatchMovie = async (req, res, next) => {
  ...
  try {
    ...
    const insertMovieId =
      await tables.usersHasToWatch.toWatchMovie(toWatchMovie);
...
const addToWatchSerie = async (req, res, next) => {
  // Récupérer l'ID utilisateur et de film depuis les paramètres de la route
  ...
  try {
    ...
    // Insérer le likedMovie dans la base de données
    const insertSerieId =
      await tables.usersHasToWatch.toWatchSerie(toWatchSerie);
...
const destroyToWatchMovie = async (req, res, next) => {
  const { userId, movieId } = req.params;
  ...
  try {
    ...
    const unToWatchMovie = {
      user_id: userId,
      movie_id: movieId,
    };

    const deleted =
      await tables.usersHasToWatch.deleteToWatchMovie(unToWatchMovie);
...
const destroyToWatchSerie = async (req, res, next) => {
  const { userId, serieId } = req.params;
  ...
  try {
    ...
    const unToWatchSerie = {
      user_id: userId,
      serie_id: serieId,
    };

    const deleted =
      await tables.usersHasToWatch.deleteToWatchSerie(unToWatchSerie);

    return res.status(201).json({ deleted, message: 'La série a été retirée' });
  } catch (err) {
    return next(err);
  }
};
...
module.exports = {
  browse,
  readUserToWatchMovies,
  readUserToWatchSeries,
  addToWatchMovie,
  addToWatchSerie,
  destroyToWatchMovie,
  destroyToWatchSerie,
