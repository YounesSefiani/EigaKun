...
class userSeenManager extends AbstractManager {
  constructor() {
    // Call the constructor of the parent class (AbstractManager)
    // and pass the table name "likedMovie" as configuration
    super({ table: 'usersHasSeen' });
  }

  // The C of CRUD - Create operation

  async hasSeenMovie(movie) {
  ...
  async readSeenMovies(userId) {
    ...
    return {
      user: userRows[0],
      seenMovies: movieRows,
    };
  }

  async readSeenSeries(userId) {
    // Execute the SQL SELECT query to retrieve a specific likedMovie by its ID
    const [userRows] = await this.database.query(
      `SELECT id, pseudo FROM users WHERE id = ?`,
      [userId]
...
module.exports = userSeenManager;
